...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.virtual_spaces: list[VirtualSpace] = []
        ...
        saved_active_monitors, saved_positions = (
            MonitorConfig.load()
        )
        ...
        self.monitor_positions: dict[str, QPointF] = (
            saved_positions
        )
        ...
        if not self.virtual_spaces:
            initial_space = self._create_space()
            self._space_selected(initial_space)
            initial_scene = self._create_scene(initial_space)
            self._scene_selected(initial_scene)
    ...
    def _setup_ui(self) -> None:
        ...
        self.add_text_button = QPushButton(
            "+ Texto"
        )
        ...
        self.delete_source_button = QPushButton(
            "Eliminar"
        )

        self.delete_source_button.setEnabled(
            False
        )

        self.activate_scene_button = QPushButton(
            "▶ Activar en Programa"
        )

        self.activate_scene_button.setToolTip(
            "Envía la escena que estás editando al Programa "
            "(salida en vivo) del espacio seleccionado, sin "
            "afectar lo que se muestra para los demás espacios."
        )
        ...
        self.add_url_button.clicked.connect(
            self.add_url_source
        )

        self.delete_source_button.clicked.connect(
            self.delete_selected_source
        )

        self.activate_scene_button.clicked.connect(
            self._activate_selected_scene
        )
        ...
        self.properties_panel.object_changed.connect(
        ...
        self.spaces_panel = EntityListPanel(
            "Espacios virtuales"
        )

        self.scenes_panel = EntityListPanel(
            "Escenas"
        )
        ...
        bottom_splitter.setStretchFactor(
            0,
            1,
        )
        ...
        self.spaces_panel.item_selected.connect(
            self._space_selected
        )

        self.scenes_panel.item_selected.connect(
            self._scene_selected
        )

        self.spaces_panel.item_renamed.connect(
            self._rename_space
        )

        self.scenes_panel.item_renamed.connect(
            self._rename_scene
        )

        self.sources_panel.item_renamed.connect(
            self._rename_source
        )

        self.sources_panel.item_selected.connect(
        ...
        # CREACIÓN
        # ======================================================
        self.spaces_panel.item_created.connect(
            self._create_space
        )

        self.scenes_panel.item_created.connect(
            self._create_scene
        )

        self.sources_panel.item_created.connect(
            self._create_source
        )
        ...
        self.spaces_panel.item_deleted.connect(
            self._delete_space
        )

        self.scenes_panel.item_deleted.connect(
            self._delete_scene
        )

        self.sources_panel.item_deleted.connect(
            self._delete_source
        )
    ...
    def open_settings(self) -> None:
        ...
        self.active_monitor_names = (
            dialog.selected_monitor_names()
        )

        self.monitor_positions = (
            dialog.selected_monitor_positions()
        )
    ...
    def _refresh_workspace_positions(
        self,
    ) -> None:
    ...
    def add_image_source(self) -> None:
        ...
        source = SourceDefinition(
            name=(
                f"Imagen "
                f"{len(self.selected_scene.sources) + 1}"
            ),
            type="imagen",
            x=0,
            y=0,
        ...
        self.selected_scene.sources.append(
            source
        )

        self._refresh_sources_for_scene(
            self.selected_scene
        )
    ...
    def delete_selected_source(self) -> None:
        ...
        source_item = (
            self.monitor_view.get_selected_source()
        )

        if source_item is None:
            return

        source_definition = getattr(
            source_item,
            "source_definition",
            None,
        )
    ...
    def _source_selection_changed(self) -> None:
        ...
        self.properties_panel.set_object(
            getattr(source_item, "source", None)
        )
    ...
    def _create_space(self):
        ...
        self._scale_space_to_monitor(
            space,
            available_monitor,
        )

        self.virtual_spaces.append(
            space
        )
    ...
    def _space_selected(
        ...
        # Seleccionar un espacio SOLO debe mostrar sus propiedades
        # (tamaño, monitor asignado). Un espacio es únicamente el
        # lienzo/marco: no debe alterar qué escena se está editando
        # ni reconstruir las fuentes en el lienzo de edición.
    ...
    def _refresh_spaces(self):
        for space in self.virtual_spaces:
            self._last_space_monitor_names.setdefault(
                id(space),
                getattr(space, "monitor_name", None),
            )
        ...
        if (
            self.selected_space is not None
            and self.selected_space in self.virtual_spaces
        ):
            self.spaces_panel.list_widget.setCurrentRow(
                self.virtual_spaces.index(
                    self.selected_space
                )
            )
    ...
    # ESCENAS
    # ==========================================================
    # Las escenas viven en una lista global (self.scenes),
    # independiente de los espacios virtuales. Cada espacio solo
    # referencia, mediante `active_scene`, cuál escena debe
    # renderizar; nunca "posee" ni filtra las escenas.

    def _create_scene(self, space=None):
        # `space` solo se usa para el arranque inicial (asignar la
        # primera escena de bootstrap a un espacio recién creado).
        # El botón "+" del panel de escenas nunca debe activar la
        # escena nueva en ningún espacio: eso requiere una acción
        # explícita (ver `activate_scene_for_space`).
        scene = Scene(
            name=(
                f"Escena "
                f"{len(self.scenes) + 1}"
        ...
        self.scenes_panel.list_widget.setCurrentRow(
            len(self.scenes) - 1
        )
    ...
    def _scene_selected(
        ...
        # Seleccionar una escena solo sirve para EDITARLA/PREVISUALIZARLA.
        # Nunca cambia qué está "en vivo" (Programa) para ningún espacio:
        # eso solo ocurre mediante `activate_scene_for_space`, invocado
        ...
        self.monitor_view.render_scene_for_editing(
            scene
        )
    ...
    def _refresh_scenes(self) -> None:
        ...
        self.scenes_panel.set_items(
            self.scenes,
            lambda scene: scene.name,
        )
    ...
    def _create_source(self) -> None:
        ...
        handlers[source_kind]()


    def _refresh_sources_for_scene(
        self,
        scene: Scene | None,
    ) -> None:
    ...
    def _source_selected(
        ...
        # Seleccionar una fuente solo debe mostrar sus propiedades.
        # NUNCA debe reconstruir el lienzo de edición (eso destruía
        # reproductores de video/URL en curso y ocultaba las demás
        # fuentes momentáneamente).
        self.properties_panel.set_object(
            source
        )
    ...
    def _delete_source(
        ...
        if source in self.selected_scene.sources:
            ...
            self.selected_scene.sources.remove(
                source
            )
    ...
    def _scale_space_to_monitor(
        ...
        monitor = next(
            (m for m in self.monitors if m.name == monitor_name),
            None,
        )
        if monitor is None:
            return
        ...
        target_width = max(1, geometry.width())
        target_height = max(1, geometry.height())
        old_width = max(1, space.width)
        old_height = max(1, space.height)

        scale_x = target_width / old_width
        scale_y = target_height / old_height

        for scene in getattr(space, "scenes", []):
            for source in getattr(scene, "sources", []):
                source.x = round(source.x * scale_x)
                source.y = round(source.y * scale_y)
                source.width = max(1, round(source.width * scale_x))
                source.height = max(1, round(source.height * scale_y))

        space.width = target_width
        space.height = target_height
    ...
    def _property_changed(
        ...
        if isinstance(
            ...
            previous_monitor = self._last_space_monitor_names.get(
                id(obj),
            )

            if previous_monitor != obj.monitor_name:
                self._scale_space_to_monitor(
                    obj,
                    obj.monitor_name,
                )

            self._last_space_monitor_names[
                id(obj)
            ] = obj.monitor_name
    ...
    def activate_scene_for_space(
        self,
        space: VirtualSpace,
        scene: Scene,
    ) -> None:
        # Único mecanismo explícito para poner una escena "en vivo"
        # (Programa) para un espacio determinado. Solo actualiza la
        # salida de ESE espacio; no toca lo que ya se reproduce en
        # los demás espacios.
        if scene not in self.scenes:
            return
    ...
    def _update_activate_button_state(self) -> None:
        self.activate_scene_button.setEnabled(
            self.selected_space is not None
            and self.selected_scene is not None
        )

    def _activate_selected_scene(self) -> None:
        """Envía al Programa (salida en vivo) la escena que se está
        editando actualmente, para el espacio actualmente
        seleccionado. Es la acción explícita que el usuario dispara
        con el botón "Activar en Programa".
        """

        if self.selected_space is None or self.selected_scene is None:
            return

        self.activate_scene_for_space(
            self.selected_space,
            self.selected_scene,
        )
